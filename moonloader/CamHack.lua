...
function load_settings()
    ...
    if not doesFileExist(path) then
		...
		print('[CamHack V2] Файл с настройками не найден, использую стандартные настройки!')
    else
        ...
        if file then
			...
			if #contents == 0 then
				settings = default_settings
				print('[CamHack V2] Не удалось открыть файл с настройками, использую стандартные настройки!')
			else
				...
				if result then
					...
					print('[CamHack V2] Настройки успешно загружены!')
				else
					print('[CamHack V2] Не удалось открыть файл с настройками, использую стандартные настройки!')
				end
			end
...
function save_settings()
    ...
    if file then
        ...
        print('[CamHack V2] Не удалось сохранить настройки скрипта, ошибка: ', errstr)
        return false
    end
end
...
function check()
	if ((isMonetLoader()) and (settings.general.camhack_type == 1 or camhack_type[0] == 1)) then
		sampAddChatMessage('{ff0000}[CamHack V2] {ffffff}Нельзя использовать Hotkeys на MonetLoader, изменяю на Joystick!',-1)
	...
	elseif ((not isMonetLoader()) and (settings.general.camhack_type == 2 or camhack_type[0] == 2 or settings.general.camhack_type == 3 or camhack_type[0] == 3)) then
		sampAddChatMessage('{ff0000}[CamHack V2] {ffffff}Нельзя использовать Joystick на MoonLoader, изменяю на Hotkey!',-1)
...
function main()
	...
	sampAddChatMessage('{ff0000}[INFO] {ffffff}Скрипт "CamHack" загружен и готов к работе! Автор: MTG MODS | Версия: 2.0 | Используйте {00ccff}/cmh',-1)

	if sampIsLocalPlayerSpawned() then
		-- выключение ников в режиме камхака
	...
	while true do 
		...
		if camhack_active then 	
			...
			if isMonetLoader() and settings.general.camhack_type == 2 or settings.general.camhack_type == 3 then -- виджеты для джойстика
			...
			if isMonetLoader() and settings.general.camhack_type == 2 then -- джойстик в 8 разных сторон
			...
			if isMonetLoader() and settings.general.camhack_type == 3 then -- джойстик в любую сторону, баганный
			...
			if isMonetLoader() and settings.general.camhack_type == 2 or settings.general.camhack_type == 3 then -- вырубить кмх виджетом
...
if not isMonetLoader() then
	function hidenicks(status) -- func hide nicks (FR)
		if pStSet and NTdist and NTwalls then
			...
			sampAddChatMessage('{ff0000}[CamHack V2] {ffffff}Не удалось переключить видимость ников, скорее всего вы не заспавнились!',-1)
		end
	end
end
...
require "samp.events".onPlayerChatBubble = function(player_id, color, distance, duration, message) -- чат над головой
	-- по дефолту отключён, на некоторых серверах запрещают юзать
...
function onScriptTerminate(script, quit) 
	if script == thisScript() and not quit then
		if camhack_active then camhack_off() end -- в случае вылета скрипта выйти из режима свободной камеры
		if camhack_active and not isMonetLoader() then hidenicks(0) end
		thisScript():reload() -- перезагрузка 
	end
end
